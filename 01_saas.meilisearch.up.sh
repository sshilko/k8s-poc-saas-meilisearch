...
KEY="$NAME.$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)"
KEY=$KEY NAME=$NAME DNS=$DNS envsubst < $WORK_DIR/manifests/product/meilisearch/meilisearch-service.yaml.tmpl | kubectl apply --namespace=$NAME -f -

echo "-------------- Your MasterKey is '$KEY'"
echo "-------------- Your Endpoint  is 'http://${NAME}.meilisearch.${DNS}'"
...
curl -v "${NAME}.meilisearch.${DNS}/keys" -H "Authorization: Bearer $KEY"